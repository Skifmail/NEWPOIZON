...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('kash/poizon_sync_service.log', encoding='utf-8')
    ]
)
...
class WooCommerceService:
    ...
    def _create_variations(self, product_id: int, product: PoisonProduct, settings: SyncSettings, use_color_attribute: bool = True):
        ...
        ОПТИМИЗАЦИЯ: НЕ загружает изображения для вариаций - использует основные изображения товара.
        ...
        logger.info(f"  Создание {len(product.variations)} вариаций (без загрузки изображений - используем основные)...")
        start_time = time.time()
        ...
        def create_single_variation(idx_var_tuple):
            ...
            try:
                ...
                # ОПТИМИЗАЦИЯ: НЕ загружаем изображения для вариаций
                # Вариации автоматически наследуют изображения родительского товара в WooCommerce
                # Экономия: 0 дополнительных файлов на вариации!
