...
// Добавлено для решения проблемы превышения лимита файлов на хостинге
// ========================================================================
...
function optimize_woocommerce_image_sizes($sizes) {
    ...
    return array(
        ...
        // 4. Размер для галереи миниатюр (150x150) - используем существующий!
...
function optimize_remove_theme_image_sizes($sizes) {
    ...
    // НЕ удаляем woocommerce_gallery_thumbnail - он нужен для галереи!
    
    return $sizes;
}

// Улучшаем качество JPEG для миниатюр (по умолчанию 82%, ставим 95%)
add_filter('jpeg_quality', function() { return 95; });
add_filter('wp_editor_set_quality', function() { return 95; });

// ========================================================================
// КОНЕЦ ОПТИМИЗАЦИИ
// РЕЗУЛЬТАТ: С 15 размеров на изображение → 4 размера (экономия 73% файлов!)
// Размеры: 150x150 (галерея), 300x300 (каталог), 600x600 (товар), оригинал
// СЛЕДУЮЩИЕ ШАГИ:
// 1. Перегенерировать thumbnails через WP-CLI для лучшего качества
// 2. Удалить старые файлы через плагин "Media Cleaner"
// ========================================================================
